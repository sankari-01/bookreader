...
class NarrationExpander:
    ...
    def theatricalize(cls, text, lang='en'):
        ...
        try:
            ...
            if enhanced and "AI Error:" not in enhanced and "<" in enhanced:
    ...
    def _manual_theater(cls, text, lang='en'):
        ...
        """
        if not text: return ""
        # Escape XML special characters only if they aren't already escaped
        ...
        for i, sent in enumerate(sentences):
            ...
            # Ensure the sentence ends with a period if it was split
            sent_cleaned = sent.strip()
            if not sent_cleaned.endswith(('.', '!', '?')):
                 sent_cleaned += "."
            ssml_parts.append(f'<prosody pitch="{pitch}" rate="{rate}">{sent_cleaned}</prosody>')
